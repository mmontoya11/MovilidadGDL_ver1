...
public class DialogManager {
	...
	static void showBluetoothDialog1(Context context, final Set<BluetoothDevice> obj) {
		final String[] items = new String[obj.size()];
		int index = 0;
		for (BluetoothDevice device : obj) {
			items[index++] = device.getAddress();
		}
		...
		new AlertDialog.Builder(context).setTitle("Paired Bluetooth printers").setItems(items, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						Reimprimir.mBixolonPrinter.connect(items[which]);
						
					}
				}).show();
	}
	
	static void showBluetoothDialog2(Context context, final Set<BluetoothDevice> obj) {
		final String[] items = new String[obj.size()];
		int index = 0;
		for (BluetoothDevice device : obj) {
			items[index++] = device.getAddress();
		...
		new AlertDialog.Builder(context).setTitle("Paired Bluetooth printers").setItems(items, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						ImprimirTicket.mBixolonPrinter.connect(items[which]);
